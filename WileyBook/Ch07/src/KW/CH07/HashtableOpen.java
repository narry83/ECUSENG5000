...
//    private static final Entry DELETED = new Entry(null, null);
	 ...
	 * @return The position of the target or the first empty slot if the target is
	 *         not in the table.
	 */
	 ...
	 *       to the argument value and numKeys is not changed. If the LOAD_THRESHOLD
	 ...
	 * 
	 * @post The size of table is doubled and is an odd integer. Each nondeleted
	 ...
	 * @return The value associated with this key, or null if the key is not in the
	 *         table.
	 */
	@Override
	...
	private static int find(Object[] array, Object key) {
		...
		// Increment index until an empty slot is reached
		// or the key is found.
		while ((array[index] != null) && (!key.equals(array[index]))) {
			index++;
			// Check for wraparound.
