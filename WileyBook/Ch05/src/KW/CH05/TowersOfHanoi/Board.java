...
public class Board implements Runnable {
    ...
    public Board(int totalWidth, int totalHeight, int numRings,
        ...
        initialized = true;
        int ringWidth = (int) (0.9 * (double) width / 3.0);
        int pegThick = (int) (0.1 * (double) width / 3.0);
        thePegs[0] = new Peg(pegThick, height, width / 2 - width / 3,
                height, "A", ringWidth,
                (int) (0.2 * (double) ringWidth),
                numRings, true);
        thePegs[1] = new Peg(pegThick, height, width / 2,
                height, "B", ringWidth,
                (int) (0.2 * (double) ringWidth),
                numRings, false);
        thePegs[2] = new Peg(pegThick, height, width / 2 + width / 3,
                height, "C", ringWidth,
                (int) (0.2 * (double) ringWidth),
                numRings, false);
    }
     ...
     * @param totalWidth The new width.
     * @param totalHeight The new height.
     */
    public synchronized void resize(int totalWidth, int totalHeight) {
        width = totalWidth;
        height = totalHeight - BASE_THICK;
        int ringWidth = (int) (0.9 * (double) width / 3.0);
        int pegThick = (int) (0.1 * (double) width / 3.0);
        thePegs[0].changeSize(pegThick, height, width / 2 - width / 3,
                height, ringWidth,
                (int) (0.2 * (double) ringWidth));
        thePegs[1].changeSize(pegThick, height, width / 2,
                height, ringWidth,
                (int) (0.2 * (double) ringWidth));
        thePegs[2].changeSize(pegThick, height, width / 2 + width / 3,
                height, ringWidth,
                (int) (0.2 * (double) ringWidth));
    }
