...
public class RecursiveMethods {
    ...
    /**
     * Class that has an iterative method for calculating Fibonacci numbers
     * that uses an ArrayList that saves each Fibonacci number as it is 
     * calculated.
     */
    public static class Fibonacci {
        private final List<Long> fiboNumbs;
        public Fibonacci() {
            fiboNumbs = new ArrayList<>();
            fiboNumbs.add((long)0);
            fiboNumbs.add((long)1);
        }
        /**
         * Calculate the ith Fibonacci number
         * @param i The index of the number desired
         * @return The ith Fibonacci number
         */
        public long fibo(int i) {
            // Get the index of the largest Fibonacci number we have
            int j = fiboNumbs.size() - 1;
            if (i <= j) return fiboNumbs.get(i);
            while (j < i) {
                fiboNumbs.add(fiboNumbs.get(j-1) + fiboNumbs.get(j));
                j = fiboNumbs.size() - 1;
            }
            return fiboNumbs.get(i);
        }
    }
    /*</exercise>*/
     ...
     * @return The subscript of target if found; otherwise -1
     */
    private static int linearSearchReverse(Object[] items,
            Object target, int posLast) {
        if (posLast == -1) {
            return -1;
        } else if (target.equals(items[posLast])) {
            return posLast;
        } else {
            return linearSearchReverse(items, target, posLast - 1);
        }
    }
